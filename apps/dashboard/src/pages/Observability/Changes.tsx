...
function getFileIcon(filePath: string): { icon: React.ElementType; color: string } {
  ...
  return { icon: FileEdit, color: 'theme-text-muted' };
}
...
export function Changes({ onNavigate: _onNavigate }: ChangesProps = {}) {
  ...
  return (
    <div
      className="flex flex-1 flex-col min-h-0 theme-bg-base theme-text-primary"
    >

      {/* ── Header ─────────────────────────────────────────────────── */}
      <div
        className="flex items-center justify-between px-5 py-3.5 border-b theme-border theme-sidebar"
      >
        <div className="flex items-center gap-2.5">
          <GitCommitHorizontal className="h-[15px] w-[15px] shrink-0 theme-text-muted" strokeWidth={2} />
          <span className="text-[13.5px] font-medium tracking-tight theme-text-primary">
            File Changes
          </span>

          {changesCount > 0 && (
            <span className="rounded-full px-2 py-0.5 text-[11px] font-medium border theme-border theme-bg-overlay-md theme-text-muted">
              {changesCount}
            </span>
          )}
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            disabled={loading}
            className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] font-medium transition-colors border theme-border theme-bg-overlay theme-text-muted"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-[11px] font-medium transition-colors disabled:opacity-40 border theme-border theme-bg-overlay theme-text-muted"
          >
      ...
      <div className="px-4 py-2.5 border-b theme-border">
        <div className="relative">
          <Search
            className="absolute left-2.5 top-1/2 h-3 w-3 -translate-y-1/2 shrink-0 theme-text-faint"
            strokeWidth={2}
          />
          <input
            type="text"
            ...
            className="w-full rounded-lg py-1.5 pl-8 pr-3 text-[12px] focus:outline-none focus:ring-1 focus:ring-blue-500/50 border theme-border theme-bg-overlay theme-text-primary"
          />
          {searchQuery && (
            <button
              type="button"
              onClick={() => setSearchQuery('')}
              className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[10px] theme-text-muted"
            >
              Clear
            </button>
          )}
      ...
      {error && (
        <div
          className="flex items-center gap-2 px-4 py-2 text-[11px] border-b theme-border theme-text-muted"
        >
      ...
      <div className="flex flex-1 flex-col overflow-y-auto">
        {loading && !data ? (
          <div className="flex items-center justify-center gap-2 p-10 theme-text-muted">
          ...
          filteredChanges.map((change) => {
            ...
            return (
              <div
                key={change.filePath}
                className="flex items-center gap-3 px-4 py-2.5 transition-colors hover:theme-bg-overlay border-b theme-border"
              >
                <Icon className={`h-[15px] w-[15px] shrink-0 ${color}`} strokeWidth={1.75} />

                <div className="flex min-w-0 flex-1 flex-col">
                  <span
                    className="truncate  text-[12.5px] font-medium theme-text-primary"
                  >
                    {fileName}
                  </span>
                  {dir && (
                    <span
                      className="truncate  text-[11px] theme-text-faint"
                    >
                      {dir}/
                    </span>
                  )}
                ...
                <div className="flex shrink-0 items-center gap-2">
                  <span className="rounded px-1.5 py-0.5 text-[10px] border theme-border theme-bg-overlay theme-text-faint">
                    {change.eventType}
                  </span>

                  <span className="w-14 text-right text-[11px] theme-text-muted">
                  ...
                  <button
                    ...
                    title="Copy path"
                    className="flex h-6 w-6 items-center justify-center rounded transition-colors border theme-border theme-bg-overlay theme-text-faint"
                  >
                    {isCopied
